...
export default function TrashIcon({ onClick, selected }: Props) { 

    const [color, setColor] = useState(selected ? '#ffffff' : '#000000');

    useEffect(() => { 
        setColor(selected ? '#ffffff' : '#000000')
    }, [selected])

    return <Wrapper>
            <svg 
                onMouseEnter={() => setColor('#a62727')} 
                onMouseLeave ={() => setColor(selected ? '#ffffff' : '#000000')} 
                onClick={onClick}
